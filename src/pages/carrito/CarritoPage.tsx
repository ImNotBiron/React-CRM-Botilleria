...
const toggleTheme = useThemeStore((s) => s.toggleTheme);
  ...
  const totalExento = useMemo(
    () =>
      carrito.reduce(
        (acc, item) =>
          item.exento_iva === 1 ? acc + item.precio_producto * item.cantidad : acc,
        0
      ),
    [carrito]
  );
  ...
  const totalAfecto = useMemo(
    () =>
      carrito.reduce(
        (acc, item) =>
          item.exento_iva === 0 ? acc + item.precio_producto * item.cantidad : acc,
        0
      ),
    [carrito]
  );
  ...
  const confirmarCantidad = () => {
    ...
    if (itemSeleccionado) {
      setCarrito((prev) =>
        prev.map((i) =>
          i.id === itemSeleccionado.id ? { ...i, cantidad: nueva } : i
        )
      );
    }
  ...
  const agregarPago = () => {
    const monto = parseInt(nuevoPagoMonto);
    ...
    if (esNoEfectivo && totalNoEfectivo + monto > totalAfecto) {
      mostrarAlerta(
        "El monto con este método excede el total afecto",
        "warning"
      );
      return;
    }
    ...
    if (monto > saldoRestante) {
      mostrarAlerta("Monto excede saldo restante", "warning");
      return;
    }
  ...
  const confirmarVenta = async () => {
    if (saldoRestante !== 0) {
      mostrarAlerta("Aún queda saldo pendiente", "warning");
      return;
    }

    if (!user) {
      mostrarAlerta("Usuario no válido", "error");
      return;
    }
    ...
    try {
      ...
      mostrarAlerta("Venta registrada", "info");
      setCarrito([]);
      cerrarModalPago();
    } catch (error) {
      ...
      <Dialog open={openPago} onClose={cerrarModalPago} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          ...
          <Box sx={{ mb: 2 }}>
            ...
            >
              <Typography fontWeight={700}>Total venta</Typography>
              <Typography fontWeight={700}>{formatCLP(totalGeneral)}</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ mt: 1, display: "flex", justifyContent: "space-between" }}>
            ...
            <Typography
              fontWeight={700}
              color={saldoRestante === 0 ? theme.palette.success.main : theme.palette.error.main}
            >
